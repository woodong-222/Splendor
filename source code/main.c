...
void get_jowel(int current_player, int nob_card[], int dev_card1[], int dev_card2[], int dev_card3[])//플레이어 토큰 연산 및 전체 토큰 연산
{
	//흰,파,빨,초,갈
	int w = 0;
	int b = 0;
	...
	printf("                                                            │ player%d의 차례입니다.                                                                                                 │\n", current_player + 1);
	printf("                                                            │ 보석 토큰 가져오기를 선택했습니다.                                                                                    │\n");
	printf("                                                            │ 가져올 토큰을 순서대로 입력하세요.(흰,파,빨,초,갈,노)                                                                 │\n");
	printf("                                                            │                                                                                                                       │\n");
	printf("                                                            │                                                                                                                       │\n");
	printf("                                                            └───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘\n");
	...
	if (total == 2)
	{
		while ((white - w < 2) || ((blue - b < 2) || ((red - r) < 2 || ((green - gr < 2) || ((brown - br < 2))))))//하나를 2개가져올때 토큰 잔량 4개 이상인지 검사		
		{
			system("cls");
			boardpan(nob_card, dev_card1, dev_card2, dev_card3);
			printf("                                                            ┌───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┐\n");
			printf("                                                            │ player%d의 차례입니다.                                                                                                 │\n", current_player + 1);
			printf("                                                            │ 어느 토큰 잔량이 4개 미만이라 가져올 수 없습니다.                                                                     │\n");
			printf("                                                            │ 다시 토큰을 입력하세요.                                                                                               │\n");
			printf("                                                            │                                                                                                                       │\n");
			printf("                                                            │                                                                                                                       │\n");
			printf("                                                            └───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘\n");
	...
	while (total > 10)
	{
		int tr_w, tr_b, tr_r, tr_gr, tr_br;//버릴 토큰 받을 변수
		system("cls");
		boardpan(nob_card, dev_card1, dev_card2, dev_card3);
		printf("                                                            ┌───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┐\n");
		printf("                                                            │ player%d의 차례입니다.                                                                                                 │\n", current_player + 1);
		printf("                                                            │ 흰색:%d   파란색:%d  빨간색:%d  초록색:%d  갈색:%d                                                                    │\n", w, b, r, gr, br);
		printf("                                                            │ 플레이어 토큰이 %d개를 초과하였습니다. 가져온 토큰 중 버릴 토큰을 입력하세요.                                         │\n", total - 10);
		printf("                                                            │ ex) 1 0 0 0 0                                                                                                         │\n");
		printf("                                                            │ (흰 1 파 1 빨 1 초 0 갈 0을 가져왔을때 총 토큰이 11개인경우)                                                          │\n");
		printf("                                                            └───────────────────────────────────────────────────────────────────────────────────────────────────────────────────────┘\n");
		scanf_s("%d %d %d %d %d", &tr_w, &tr_b, &tr_r, &tr_gr, &tr_br);
		total -= tr_w + tr_b + tr_r + tr_gr + tr_br;
		w -= tr_w; b -= tr_b; r -= tr_r; gr -= tr_gr; br -= tr_gr;//가져온 토큰에서 넘치는 만큼 버리기
	}


	white -= w; blue -= b; red -= r; green -= gr; brown -= br;//창고에서 가져오는만큼 빼기
	player[current_player].white += w; player[current_player].blue += b; player[current_player].red += r; player[current_player].green += gr; player[current_player].brown += br;//가져온 만큼 플레이어 토큰에 더하기
	current_player++;
